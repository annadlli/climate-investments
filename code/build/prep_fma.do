...
Revision: Keep all HMA programs, calculate number of properties under HMGP,  
...
keep if inlist(programarea, "FMA", "SRL") 
...
* Import HMA home-elevation grants (all programs; clean_fma keeps programarea as the flag)
use "`data'/clean/fma_elevation.dta", clear
...
* HMGP properties by county (09-12 edited)
// Note: counts only; the spending and BCR variables stay FMA-specific below
preserve
    keep if programarea == "HMGP"
    gen n_props = cond(project_merge == 2, n_properties, n_properties_rec)
    collapse (sum) hmgp_n_properties = n_props (firstnm) state county, by(state_code county_code)
    tempfile hmgp
    save `hmgp'
restore
...
* Add the HMGP property counts (09-12 edit)
// Note: counties with HMGP but no FMA elevations enter here with fma_* missing
merge 1:1 state_code county_code using `hmgp', nogen
replace hmgp_n_properties = 0 if mi(hmgp_n_properties)
...
label var hmgp_n_properties "Properties elevated under HMGP grants in county" 
